...
public class Snake {
    public void reset() {
        arrPartSnake.clear();  // 清空蛇的部分

        // 重新初始化蛇的頭部
        arrPartSnake.add(new PartSnake(bm_head_right, 0, 0));

        // 重新初始化蛇的身體部分
        for (int i = 1; i < length - 1; i++) {
            arrPartSnake.add(new PartSnake(bm_body_horizontal, arrPartSnake.get(i - 1).getX() - GameView.size, 0));
        }
